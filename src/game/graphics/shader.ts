...
export class Shader {
  ...
  constructor(gl: WebGLRenderingContext, shaderData: ShaderData) {
    ...
    this.webGlShaderId = getSafe(gl.createShader(shaderData.type), `Couldn't create the shader`);
    ...
    if (!gl.getShaderParameter(this.webGlShaderId, gl.COMPILE_STATUS)) {
      throw new Error(`Couldn't compile the shader. ${gl.getShaderInfoLog(this.webGlShaderId)}`);
    }
  }
